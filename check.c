...
void check_alloc_freelist(void *mem) {
    ...
    assert(ctx->used == sizeof(ma_ctx) + 10 + sizeof(ma_alloc_freelist) + sizeof(ma_alloc_freelist_entry));
}
...
void check_alloc_freelist_repeated(void *mem) {
    ...
    assert(ctx->used == sizeof(ma_ctx) + 10 * 10 + sizeof(ma_alloc_freelist) + 10 * sizeof(ma_alloc_freelist_entry));
}
...
void check_alloc_freelist_bestfit(void *mem) {
    ...
    addr1 = ma_alloc(ctx, 5);
    addr2 = ma_alloc(ctx, 10);
    assert(ctx->used == sizeof(ma_ctx) + 10 + 5 + sizeof(ma_alloc_freelist) + 2 * sizeof(ma_alloc_freelist_entry));
...
void check_alloc_pool(void *mem) {
    ma_ctx *ctx = ma_create_allocator_pool(mem, 1024, 10);
    assert(ctx->used == sizeof(ma_ctx) + sizeof(ma_alloc_pool));

    void *addr = ma_alloc(ctx, 10);
    assert(ctx->used == sizeof(ma_ctx) + sizeof(ma_alloc_pool) + sizeof(ma_alloc_pool_entry) + 10);
    ma_free(ctx, addr);
...
void check_alloc_pool_repeated(void *mem) {
    ...
    for (int i = 0; i < 10; i++) {
        addr[i] = ma_alloc(ctx, 10);
        assert(ctx->used == sizeof(ma_ctx) + sizeof(ma_alloc_pool) + (sizeof(ma_alloc_pool_entry) + 10) * (i + 1));
    }

    for (int i = 0; i < 10; i++) {
        ma_free(ctx, addr[i]);
        assert(ctx->used == sizeof(ma_ctx) + sizeof(ma_alloc_pool) + (10 - 1 - i ) * (sizeof(ma_alloc_pool_entry) + 10));
    }
